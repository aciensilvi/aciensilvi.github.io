class shopItem{constructor(title,amount,short_description,link1,link2,linkCustom,price1,price2){this.title=title;this.amount=amount;this.short_description=short_description;this.link1=link1;this.link2=link2;this.linkCustom=linkCustom;this.price1=price1;this.price2=price2;this.linkToItem=title.toLowerCase().replace(" ","_")}}tabernas=new shopItem("TABERNAS HAT",4,"Regenerative materials","https://buy.stripe.com/28o15M0OnabT1ag28f","https://buy.stripe.com/6oE7ua0Onfwd6uAfZ4","https://docs.google.com/forms/d/e/1FAIpQLSfwUTVYOcDEiWB0dPzPrBpUZsiKROyBnfFhusImUwh3b8a5lg/viewform","┬ú900","┬ú850");carnation=new shopItem("CARNATION HAT",3,"Regenerative materials","https://buy.stripe.com/fZeg0GeFdgAhaKQ4gp","https://buy.stripe.com/14keWCdB9ck13io8wE","https://docs.google.com/forms/d/e/1FAIpQLScnJelnosuK6A9Ne4EBDuaI-nxqRQHXSXjw_uta9oCxXcT0zg/viewform","┬ú950","┬ú900");const shopItemList=[tabernas,carnation];let columnForShopItem="";for(let item of shopItemList){columnForShopItem+=`
...
<a class="button btn btn-dark" href="https://docs.google.com/forms/d/e/1FAIpQLSetxYHsEcbEOhDrCb9IFHJFHMfxW3l8WFBmWAfnbQ0ki7kifw/viewform?usp=sf_link" target="_blank">Buy</a>
  ...
  `}class Shop extends HTMLElement{connectedCallback(){let shopPage="";for(let eachShopItem of shopItemList){var imageCount=eachShopItem.amount;let carousel_inner="";for(let i=1;i<=imageCount;i++){carousel_inner+=`<div class="carousel-item">
             <img src="images/shop/`+eachShopItem.linkToItem+`/${i}.jpg" class="d-block pic_w50 mx-auto" alt="`+eachShopItem.title+`">
           </div>`}let carousel_indicators="";for(let i=1;i<=imageCount;i++){carousel_indicators+=`
        ...
        <div class="carousel-indicators">`+carousel_indicators+`
    </div>
          <a href='`+eachShopItem.linkToItem+`' class="shop-title"><div class="carousel-inner">`+carousel_inner+`</div>
      ...
      <div class="row justify-content-md-center">`+shopPage+`</div>
      </div>
...
<div class="row justify-content-md-center">
    ...
    <p>Regenerative materials</p>
      ...
      `;document.querySelectorAll(".carousel-item").forEach(el=>{if(el.querySelector("img").getAttribute("src").includes("1")){el.classList.add("active")}});document.querySelectorAll("[aria-label^=Slide]").forEach(el=>{if(el.getAttribute("data-bs-slide-to").includes("0")){el.classList.add("active");el.setAttribute("aria-current","true")}})}}class ShopItem extends HTMLElement{connectedCallback(){const itemNameFromWeb=document.querySelector("app-shop-item").getAttribute("item");const description=document.querySelector("description");let ourItem;for(let numberInList of shopItemList){if(numberInList.title==itemNameFromWeb){ourItem=numberInList}}var imageCount=ourItem.amount;let carousel_inner="";for(let i=1;i<=imageCount;i++){carousel_inner+=`<div class="carousel-item">
      ...
      <div class="row justify-content-md-center">
        ...
        </div></div><br>`;document.querySelectorAll(".carousel-item").forEach(el=>{if(el.querySelector("img").getAttribute("src").includes("1")){el.classList.add("active")}});document.querySelectorAll("[aria-label^=Slide]").forEach(el=>{if(el.getAttribute("data-bs-slide-to").includes("0")){el.classList.add("active");el.setAttribute("aria-current","true")}});description.remove();function includeFSlightbox(file){var script=document.createElement("script");script.src=file;script.type="text/javascript";script.crossorigin="anonymous";var s=document.body.firstChild;s.parentNode.insertBefore(script,s)}includeFSlightbox("js/fslightbox.js");const selectOption=document.getElementById("selectOption");const priceElement=document.getElementById("price");const priceContainer=document.getElementById("priceContainer");const buyButton=document.getElementById("buyButton");const options={1:{price:ourItem.price1,link:ourItem.link1},2:{price:ourItem.price2,link:ourItem.link2},3:{price:"Custom Price",link:ourItem.linkCustom}};selectOption.addEventListener("change",function(){const selectedOption=selectOption.value;if(selectedOption==="3"){priceContainer.style.display="none";buyButton.href=options[selectedOption].link}else{priceElement.textContent=options[selectedOption].price;priceContainer.style.display="block";buyButton.href=options[selectedOption].link}});window.addEventListener("load",function(){const selectedOption=selectOption.value;priceElement.textContent=options[selectedOption].price;buyButton.href=options[selectedOption].link})}}customElements.define("app-shop",Shop);customElements.define("app-shop-item",ShopItem);
